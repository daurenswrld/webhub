      ...
      <div className="container">
        <div className="talent-header-flex animate-fade-in">
          <div className="section-header" style={{ marginBottom: 0 }}>
            ...
            <p style={{ maxWidth: '550px', fontSize: '0.9rem', marginTop: '8px' }}>
              Прототип нашей будущей биржи технологических проектов. Сейчас это реестр наших штатных специалистов и текущих активностей студии, готовый к масштабированию.
            </p>
          </div>
        ...
        {/* Filters bar */}
        <div className="animate-fade-in" style={{ display: 'flex', gap: '8px', marginBottom: 'var(--space-3)', flexWrap: 'wrap', marginTop: 'var(--space-2)' }}>
          {['all', 'frontend', 'backend', 'design'].map((cat) => (
            <button
              ...
              style={{
                ...
                border: '1px solid',
                borderColor: filter === cat ? 'var(--text-primary)' : 'var(--border-dim)',
          ...
          {isLoading ? (
            ...
            Array.from({ length: 3 }).map((_, i) => (
              ...
              >
                ...
                <div className="talent-meta" style={{ width: '30%', justifyContent: 'flex-end' }}>
            ...
            filteredData.map((item) => (
              <div key={item.id} className="talent-item animate-fade-in">
                <div className="talent-info">
                  <span className={`talent-status ${item.status === 'active' ? 'status-active' : 'status-pending'}`} />
                  <div className="talent-details">
                    <div className="talent-title-row">
                      <span className="talent-name">{item.name}</span>
                      <span className="talent-tag">{item.category}</span>
                    </div>
                    <div className="talent-desc" style={{ color: 'var(--text-secondary)' }}>
                      {item.role} • <span style={{ color: 'var(--text-muted)' }}>{item.experience}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="talent-meta">
                  <div className="talent-budget" style={{ textAlign: 'right' }}>
                    <div>{item.rate}</div>
                    <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}>{item.statusText}</div>
                  </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '200px', border: '1px dashed var(--border-primary)', borderRadius: 'var(--radius-sm)' }}>
              <p style={{ color: 'var(--text-muted)' }}>Нет активных позиций в данной категории.</p>
            </div>
          )}
        </div>

        {/* Info card footer */}
        <div className="animate-fade-in" style={{ marginTop: 'var(--space-3)', padding: 'var(--space-3)', backgroundColor: 'var(--bg-secondary)', border: '1px solid var(--border-primary)', borderRadius: 'var(--radius-sm)', display: 'flex', alignItems: 'flex-start', gap: '12px', textAlign: 'left' }}>
          <Briefcase size={20} style={{ color: 'var(--text-secondary)', marginTop: '2px', flexShrink: 0 }} />
          <div>
            <h4 style={{ fontSize: '0.9rem', color: 'var(--text-primary)' }}>Как нанять этих специалистов?</h4>
            <p style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', marginTop: '2px' }}>
              Все представленные профессионалы являются постоянными резидентами WebHub. Вы можете привлечь их к реализации вашего проекта под управлением нашего техлида. Опишите ваши требования в форме связи ниже.
            </p>
          </div>
        </div>
